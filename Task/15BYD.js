...
return new Promise((resolve, reject) => {
  ...
  let signdouble ={
    ...
    headers: JSON.parse(CookieVal),
    body: `nonce_str=${signInStr}&tid=16&pos=1&`,
}
  ...
  return new Promise((resolve, reject) => {
    ...
    let pophongbaoyu ={
     ...
     $.get(pophongbaoyu,async(error, response, data) =>{
       $.log('\n🔔开始查询红包雨\n')
       $.log('————pophongbaoyu————\n'+data)
       const pophongbaoyu1 = JSON.parse(data)
        if(pophongbaoyu1.code == 200) {
          if(pophongbaoyu1.hongbaoyu_show == 1 && pophongbaoyu1.hongbaoyu_count != 0){
            $.log('\n🔔红包雨准备好了，开始领取红包雨\n')
            random = Math.floor(Math.random()*(max-min+1)+min)*1000
            console.log(random);
            await $.wait(random);
  ...
  return new Promise((resolve, reject) => {
    ...
    let hongbaoyu ={
     ...
     $.post(hongbaoyu,async(error, response, data) =>{
       //$.log('\n🔔开始领取红包雨\n')
       $.log('————pophongbaoyu————\n'+data)
       const hongbaoyu1 = JSON.parse(data)
        if(hongbaoyu1.code == 200) {
            $.log('\n🎉'+hongbaoyu1.message+'红包雨 金币 +'+hongbaoyu1.jinbi+'💰\n')
           }else{
            $.log('\n🎉'+sign.message+'\n')
           }
            resolve()
  ...
  let waterclick ={
   ...
   $.post(waterclick,async(error, response, data) =>{
      ...
      if(clickwater.code == 200) {
          $.log('\n🎉'+clickwater.message+'喝水金币+ '+clickwater.jinbi+'💰\n')
           }else{
          $.log('\n⚠️喝水失败:'+clickwater.message+'\n')
           }
          resolve()
    })
   })
   ...
   $.post(sleepdone,async(error, response, data) =>{
     const donesleep = JSON.parse(data)
      ...
      if(donesleep.code == 200) {
          $.log('\n🎉'+donesleep.message+'金币+ '+donesleep.jinbi+'💰\n')
      }else{
          $.log('\n⚠️领取睡觉金币失败敗:'+donesleep.message+'\n')
      }
        resolve()
    })
   })
   ...
   $.post(taskclickDoublead,async(error, response, data) =>{
     const taskclickDouble = JSON.parse(data)
      ...
      if(taskclickDouble.code == 200) {
          ...
          $.log('\n⚠️翻倍奖励领取失败:'+taskclickDouble.message+'\n')
           }
          resolve()
    })
   })
   ...
   $.post(luckybox,async(error, response, data) =>{
     const boxlucky = JSON.parse(data)
      ...
      if(boxlucky.code == 200) {
          $.log('🎉宝箱: '+boxlucky.message+' 金币+'+boxlucky.jinbi+'\n')
