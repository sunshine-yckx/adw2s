...
if ($.isNode()) {
  ...
  if (process.env.YSMURL && process.env.YSMURL.indexOf('\n') > -1) {
  ...
  if (process.env.YSMHD && process.env.YSMHD.indexOf('\n') > -1) {
  ...
  if (process.env.YSMBODY && process.env.YSMBODY.indexOf('\n') > -1) {
  ...
  if (process.env.YSM2BODY && process.env.YSM2BODY.indexOf('\n') > -1) {
  ...
  if (process.env.YSMTX && process.env.YSMTX.indexOf('\n') > -1) {
...
//云扫码签到
function ysmsign(timeout = 0) {
  return new Promise((resolve) => {
let url = {
        url : "http:"+ysmurl.match(/http:(.*?)yunonline/)[1]+"yunonline/v1/sign_in",
      ...
      $.post(url, async (err, resp, data) => {
        try {
          const result = JSON.parse(data)
              if(result.errcode == 0){
              console.log('\n云扫码[签到]回执:成功🌝 获得金币：'+result.data.gold+'\n')
              }
             if(result.errcode == 405){
                  console.log('\n🧼来自肥皂的提示:'+result.msg+'---已经签过了。')
                  }
        } catch (e) {
          //$.logErr(e, resp);
        } finally {
